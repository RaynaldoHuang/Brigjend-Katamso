                ...
                <div class=" mt-2">
                    <div class="d-flex justify-content-end gap-2 align-items-center">
                        ...
                        <a href="{{ route('admin.carousel.create') }}" class="btn btn-primary"
                            {{ $carouselImages->count() >= 5 ? 'disabled' : '' }}>
                    ...
                    <table class="table mt-2">
                        ...
                        <tbody>
                            @foreach ($carouselImages as $carousel)
                                <tr>
                                    <td>
                                        <img src="{{ asset($carousel->image) }}" alt="Carousel Image" class="img-fluid"
                                            style="max-width: 100px">
                                    </td>
                                    <td class="fw-semibold">{{ $carousel->name }}</td>
                                    <td>
                                        @if ($carousel->action && $carousel->url)
                                            <a href="{{ $carousel->url }}" target="_blank" class="btn btn-primary">
                                                {{ $carousel->action }} </a>
                                        @else
                                            -
                                        @endif
